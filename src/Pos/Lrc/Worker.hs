...
leadersComputationDo epochId seed =
    unlessM (isJust <$> getLeaders epochId) $ do
        ...
        leaders <- GS.balanceSource $ \src -> runConduit $ src .| followTheSatoshiM seed totalStake
...
richmenComputationDo epochIdx consumers = unless (null consumers) $ do
    ...
    (richmen, richmenD) <- GS.balanceSource $ \src -> runConduit $
        src .| findAllRichmenMaybe minThreshold minThresholdD
